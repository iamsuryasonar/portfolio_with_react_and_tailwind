...
function Project({ image, projectName, projectDescription, source_code, live_url, icons, translate_y_percent }) {
    ...
    return <div className="max-w-4xl mx-6 flex flex-col items-center gap-4">
        <div className="flex flex-col items-center gap-4 p-4 bg-white rounded-lg shadow-lg md:grid md:grid-cols-5 "  >
            <div className=" max-w-96 max-h-96 overflow-hidden rounded-xl col-span-3 aspect-square">
...
function Projects() {
    return <>
        <section id='projects' className="py-24 min-h-svh w-full bg-slate-50 flex flex-col items-center gap-16 ">
            ...
            {projects.map((item) => {
                return <Project key={item.id} image={item.image} projectName={item.title} projectDescription={item.description} source_code={item.source_code} live_url={item.url} icons={item.icons} translate_y_percent={item.translate_y_percent} />
            })}
